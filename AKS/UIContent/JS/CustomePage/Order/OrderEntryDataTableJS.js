function FnEditNote(ctrl) {
    ...
    window.location.href = "/Order/ViewOrder?DocumentNumber=" + primarykey+'&IsEdit=1';
};
...
function FnViewNote(ctrl) {
    ...
    window.location.href = "/Order/ViewOrder?DocumentNumber=" + primarykey;
};
$(document).ready(function () {
    var dtinstance = $('#tblDataList').DataTable({
        columns: [
            ...
            { 'data': 'OrderStatusStr' },
            {
                'data': 'DocumentNumber', render: function (data, type, row, meta) {
                    ...
                    mbtns = mbtns + viewBtn;
                    if (row.OrderStatus<4) { mbtns = mbtns + editBtn; }
                    mbtns = mbtns + '</span>';
        ...
        bServerSide: true,
        sAjaxSource: '/Order/GetOrderList',
        "pagingType": "input",
    });
